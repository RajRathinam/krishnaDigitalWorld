...
import { SearchIcon, X, Sparkles } from 'lucide-react';
...
export function SearchModal({ children, data, onSelect }) {
  ...
  React.useEffect(() => {
    // Reset showAll when modal opens/closes
    if (!open) {
      setShowAll(false);
      setQuery('');
    }
  ...
  const displayItems = showAll ? filteredData : [];
  ...
  return (
    <Modal open={open} onOpenChange={setOpen} mobileDirection="top">
      ...
      <ModalContent className="p-0 gap-0 overflow-hidden max-w-2xl md:top-[15%] md:translate-y-0">
        ...
        <div className="flex items-center gap-2 p-3 py-3.5 border-b border-border md:hidden">
        ...
        <Command className="border-0 shadow-none hidden md:flex">
          ...
          <CommandList className="max-h-[400px]">
            ...
            {/* Show popular suggestions when not searching */}
            {!query && !showAll && (
        ...
        <div className="md:hidden max-h-[70vh] overflow-y-auto">
          {query && filteredData.length === 0 ? (
            ...
            <div className="divide-y divide-border">
              {/* Show popular suggestions on mobile when not searching */}
              {!query && !showAll && (
                <>
